...
import { store, usePluginContext } from '@grafana/data';
...
class UserStorage {
  ...
  async getItem(key) {
    ...
    if (!this.canUseUserStorage) {
      return (_a = store.get(`${this.resourceName}:${key}`)) != null ? _a : null;
    }
    ...
    try {
      ...
      if (!storageSpec) {
        return (_b = store.get(`${this.resourceName}:${key}`)) != null ? _b : null;
      }
...
export { UserStorage, _clearStorageCache, usePluginUserStorage };
