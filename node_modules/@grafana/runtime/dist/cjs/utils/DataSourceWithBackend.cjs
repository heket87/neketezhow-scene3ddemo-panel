...
var utils = require('../analytics/utils.cjs');
var config = require('../config.cjs');
var index = require('../internal/openFeature/index.cjs');
require('../services/index.cjs');
var publicDashboardQueryHandler = require('./publicDashboardQueryHandler.cjs');
var qscheck = require('./qscheck.cjs');
var queryResponse = require('./queryResponse.cjs');
var userStorage = require('./userStorage.cjs');
var dataSourceSrv = require('../services/dataSourceSrv.cjs');
var backendSrv = require('../services/backendSrv.cjs');
var live = require('../services/live.cjs');
...
var PluginRequestHeaders = /* @__PURE__ */ ((PluginRequestHeaders2) => {
  ...
  PluginRequestHeaders2["DashboardTitle"] = "X-Dashboard-Title";
  PluginRequestHeaders2["PanelTitle"] = "X-Panel-Title";
...
class DataSourceWithBackend extends data.DataSourceApi {
  constructor(instanceSettings) {
    ...
    this.userStorage = new userStorage.UserStorage(instanceSettings.type);
  }
  /**
  ...
  query(request) {
    var _a;
    if (config.config.publicDashboardAccessToken) {
      return publicDashboardQueryHandler.publicDashboardQueryHandler(request);
    }
    ...
    const queries = targets.map((q) => {
      ...
      if (q.datasource) {
        const ds = dataSourceSrv.getDataSourceSrv().getInstanceSettings(q.datasource, request.scopedVars);
        if (!ds) {
    ...
    if (config.config.featureToggles.queryServiceFromUI) {
      const allowedTypes = index.getFeatureFlagClient().getObjectValue("datasources.querier.fe-allowed-types", {
        types: []
      });
      if (qscheck.isQueryServiceCompatible(datasources, allowedTypes)) {
        url = `/apis/query.grafana.app/v0alpha1/namespaces/${config.config.namespace}/query?ds_type=${this.type}`;
      }
    }
    if (hasExpr) {
    ...
    return backendSrv.getBackendSrv().fetch({
      ...
      operators.switchMap((raw) => {
        var _a2;
        const rsp = queryResponse.toDataQueryResponse(raw, queries);
  ...
  async callHealthCheck() {
    return rxjs.lastValueFrom(
      ...
      utils.reportInteraction("datasource_health_check_completed", properties);
      return err.data;
    });
  }
  /**
...
function toStreamingDataResponse(rsp, req, getter) {
  var _a;
  const live$1 = live.getGrafanaLiveSrv();
  if (!live$1) {
    return rxjs.of(rsp);
  }
  const staticdata = [];
...
exports.ExpressionDatasourceRef = ExpressionDatasourceRef;
...
//# sourceMappingURL=DataSourceWithBackend.cjs.map
