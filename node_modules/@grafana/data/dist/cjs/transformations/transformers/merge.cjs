...
const mergeTransformer = {
  id: ids.DataTransformerID.merge,
  name: "Merge series/tables",
  description: "Merges multiple series/tables into a single serie/table",
  defaultOptions: {},
  isApplicable: (data) => {
    return data.length > 1 ? transformations.TransformationApplicabilityLevels.Applicable : transformations.TransformationApplicabilityLevels.NotApplicable;
  },
  ...
  operator: (options) => (source) => source.pipe(
    operators.map((dataFrames) => {
      ...
      const dataFrame = new MutableDataFrame.MutableDataFrame({
        refId: `${ids.DataTransformerID.merge}-${data.map((frame) => frame.refId).join("-")}`,
        fields: []
      });
