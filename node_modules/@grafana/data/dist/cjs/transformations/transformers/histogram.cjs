...
require('../../index.cjs');
var displayProcessor = require('../../field/displayProcessor.cjs');
var createTheme = require('../../themes/createTheme.cjs');
var dataFrame = require('../../types/dataFrame.cjs');
var dataFrameTypes = require('../../types/dataFrameTypes.cjs');
var numbers = require('../../utils/numbers.cjs');
var ids = require('./ids.cjs');
var joinDataFrames = require('./joinDataFrames.cjs');
var nullToValue = require('./nulls/nullToValue.cjs');
...
const histogramTransformer = {
  id: ids.DataTransformerID.histogram,
  name: "Histogram",
...
function getHistogramFields(frame) {
  var _a, _b;
  if (((_a = frame.meta) == null ? void 0 : _a.type) === dataFrameTypes.DataFrameType.HeatmapCells) {
    ...
    let denseMins = [];
    let denseMaxs = [];
    const MAX_DENSIFIED_BUCKETS = 1e3;
    ...
    return fields;
  } else if (((_b = frame.meta) == null ? void 0 : _b.type) === dataFrameTypes.DataFrameType.HeatmapRows) {
    let minVals = [];
    let maxVals = [];
    let countVals = [];
    let minVal = "0";
    frame.fields.forEach((f) => {
      if (f.type === dataFrame.FieldType.number) {
  ...
  for (const field of frame.fields) {
    if (isHistogramFrameBucketMinFieldName(field.name)) {
      ...
      xMax = field;
    } else if (field.type === dataFrame.FieldType.number) {
      counts.push(field);
    }
  }
  if (!xMax && xMin && xMin.values.length > 1) {
    let vals = xMin.values;
    let bucketSize = numbers.roundDecimals(vals[1] - vals[0], 6);
    xMax = {
      ...xMin,
...
function buildHistogram(frames, options, theme) {
      ...
      fields: frame.fields.map((field) => {
        if (field.type === dataFrame.FieldType.number) {
          ...
          if (!Number.isNaN(noValue)) {
            field = nullToValue.nullToValueField(field, noValue);
          } else {
  ...
  const getBucket = (v) => numbers.roundDecimals(incrRoundDn(v - bucketOffset, bucketSize) + bucketOffset, 9);
  ...
  for (const frame of frames) {
    for (const field of frame.fields) {
      if (field.type === dataFrame.FieldType.number) {
        ...
        count.display = displayProcessor.getDisplayProcessor({
          field: count,
          theme: theme != null ? theme : createTheme.createTheme()
        });
        counts.push(count);
  ...
  let joinedHists = joinDataFrames.join(histograms);
...
function histogramFieldsToFrame(info, theme) {
  ...
  return {
    ...
    meta: {
      type: dataFrameTypes.DataFrameType.Histogram
    },
    fields: [info.xMin, info.xMax, ...info.counts],
    refId: `${ids.DataTransformerID.histogram}`
  };
}
function joinHistograms(histograms) {
  ...
  let joined = joinDataFrames.joinDataFrames({
