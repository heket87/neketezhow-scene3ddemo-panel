...
var binaryOperators = require('../../utils/binaryOperators.cjs');
var unaryOperators = require('../../utils/unaryOperators.cjs');
var fieldReducer = require('../fieldReducer.cjs');
var matchers = require('../matchers.cjs');
var ids = require('../matchers/ids.cjs');
var ensureColumns = require('./ensureColumns.cjs');
...
const defaultNumericVizOptions = {
  reducer: fieldReducer.ReducerID.sum
};
const defaultWindowOptions = {
  reducer: fieldReducer.ReducerID.mean,
...
const defaultBinaryOptions = {
  left: { fixed: "" },
  operator: binaryOperators.BinaryOperationID.Add,
  right: { fixed: "" }
};
const defaultUnaryOptions = {
  operator: unaryOperators.UnaryOperationID.Abs,
  fieldName: ""
};
const calculateFieldTransformer = {
  id: ids$1.DataTransformerID.calculateField,
  ...
  operator: (options, ctx) => (outerSource) => {
    ...
    const operator = needsSingleFrame ? ensureColumns.ensureColumnsTransformer.operator(null, ctx) : noop.noopTransformer.operator({}, ctx);
      ...
      operators.map((data) => {
        ...
        switch (mode) {
          case "reduceRow" /* ReduceRow */:
            creator = getReduceRowCreator(lodash.defaults(options.reduce, defaultNumericVizOptions), data);
            break;
          case "cumulativeFunctions" /* CumulativeFunctions */:
            creator = getCumulativeCreator(lodash.defaults(options.cumulative, defaultNumericVizOptions), data);
            break;
          ...
          case "binary" /* BinaryOperation */:
            ...
            if (((_h = (_g = binaryOptions.left) == null ? void 0 : _g.matcher) == null ? void 0 : _h.id) && ((_i = binaryOptions.left) == null ? void 0 : _i.matcher.id) === ids.FieldMatcherID.byType) {
              const fieldType = binaryOptions.left.matcher.options;
              const operator2 = binaryOperators.binaryOperators.getIfExists(binaryOptions.operator);
          ...
          case "index" /* Index */:
            return data.map((frame) => {
              ...
              const f = {
                ...
                type: dataFrame.FieldType.number,
                values: indexArr,
        ...
        const outFrames = data.map((frame) => {
          ...
          if ((_a3 = options.alias) == null ? void 0 : _a3.length) {
...
function getWindowCreator(options, allFrames) {
  ...
  let matcher = matchers.getFieldMatcher({
    id: ids.FieldMatcherID.numeric
  });
  if (options.field) {
    matcher = matchers.getFieldMatcher({
      id: ids.FieldMatcherID.byNames,
      options: {
        names: [options.field]
      }
    });
  ...
  return (frame) => {
    ...
    if (![fieldReducer.ReducerID.mean, fieldReducer.ReducerID.stdDev, fieldReducer.ReducerID.variance].includes(options.reducer)) {
...
function getTrailingWindowValues(frame, reducer, selectedField, window) {
  ...
  for (let i = 0; i < frame.length; i++) {
    if (reducer === fieldReducer.ReducerID.mean) {
    ...
    } else if (reducer === fieldReducer.ReducerID.variance) {
      const start = Math.max(0, i - window + 1);
      const end = i + 1;
      vals.push(calculateVariance(selectedField.values.slice(start, end)));
    } else if (reducer === fieldReducer.ReducerID.stdDev) {
      const start = Math.max(0, i - window + 1);
      const end = i + 1;
...
function getCumulativeCreator(options, allFrames) {
  ...
  if (![fieldReducer.ReducerID.mean, fieldReducer.ReducerID.sum].includes(options.reducer)) {
    throw new Error(`Add field from calculation transformation - Unsupported reducer: ${options.reducer}`);
  }
  return (frame) => {
    var _a;
    ...
    for (let i = 0; i < frame.length; i++) {
      ...
      if (options.reducer === fieldReducer.ReducerID.sum) {
        vals.push(total);
      } else if (options.reducer === fieldReducer.ReducerID.mean) {
        vals.push(total / (i + 1));
      }
    }
    return vals;
...
function getReduceRowCreator(options, allFrames) {
  ...
  const info = fieldReducer.fieldReducers.get(options.reducer);
  if (!info) {
    throw new Error(`Unknown reducer: ${options.reducer}`);
  }
  const reducer = (_a = info.reduce) != null ? _a : fieldReducer.doStandardCalcs;
  const ignoreNulls = options.nullValueMode === data.NullValueMode.Ignore;
  const nullAsZero = options.nullValueMode === data.NullValueMode.AsZero;
  return (frame) => {
    const columns = [];
    for (const field of frame.fields) {
      if (matcher(field, frame, allFrames)) {
...
function findFieldValuesWithNameOrConstant(frame, value, allFrames, ctx) {
  ...
  if (value.matcher && value.matcher.id === ids.FieldMatcherID.byName) {
    const name = (_a = value.matcher.options) != null ? _a : "";
    for (const f of frame.fields) {
      if (name === fieldState.getFieldDisplayName(f, frame, allFrames)) {
        if (f.type === dataFrame.FieldType.boolean) {
...
function getBinaryCreator(options, allFrames, ctx) {
  const operator = binaryOperators.binaryOperators.getIfExists(options.operator);
  return (frame) => {
...
function checkBinaryValueType(value, names) {
  if (typeof value === "string") {
    if (isNaN(Number(value))) {
      return { matcher: { id: ids.FieldMatcherID.byName, options: value } };
    } else {
...
function getUnaryCreator(options, allFrames) {
  const operator = unaryOperators.unaryOperators.getIfExists(options.operator);
  return (frame) => {
    let value = [];
    for (const f of frame.fields) {
      if (options.fieldName === fieldState.getFieldDisplayName(f, frame, allFrames) && f.type === dataFrame.FieldType.number) {
        value = f.values;
      }
    }
    ...
    let sum = 0;
    if (options.operator === unaryOperators.UnaryOperationID.Percent) {
...
function getNameFromOptions(options) {
  ...
  switch (options.mode) {
    ...
    case "binary" /* BinaryOperation */: {
      ...
      const left = (_j = (_i = (_g = (_f = binary == null ? void 0 : binary.left) == null ? void 0 : _f.matcher) == null ? void 0 : _g.options) != null ? _i : (_h = binary == null ? void 0 : binary.left) == null ? void 0 : _h.fixed) != null ? _j : "";
      const right = (_o = (_n = (_l = (_k = binary == null ? void 0 : binary.right) == null ? void 0 : _k.matcher) == null ? void 0 : _l.options) != null ? _n : (_m = binary == null ? void 0 : binary.right) == null ? void 0 : _m.fixed) != null ? _o : "";
      if (/\$/.test(left) || /\$/.test(right)) {
        return "";
      }
      const operator = (_p = binary == null ? void 0 : binary.operator) != null ? _p : binaryOperators.BinaryOperationID.Add;
      return left && right ? `${left} ${operator} ${right}` : "";
    }
    case "reduceRow" /* ReduceRow */:
      {
        const r = fieldReducer.fieldReducers.getIfExists((_q = options.reduce) == null ? void 0 : _q.reducer);
        if (r) {
          return r.name;
        }
      }
...
exports.CalculateFieldMode = CalculateFieldMode;
exports.WindowAlignment = WindowAlignment;
exports.WindowSizeMode = WindowSizeMode;
exports.calculateFieldTransformer = calculateFieldTransformer;
exports.checkBinaryValueType = checkBinaryValueType;
exports.defaultWindowOptions = defaultWindowOptions;
exports.getNameFromOptions = getNameFromOptions;
//# sourceMappingURL=calculateField.cjs.map
