...
var parser = require('../../datetime/parser.cjs');
...
const convertFieldTypeTransformer = {
  id: ids.DataTransformerID.convertFieldType,
...
function convertFieldTypes(options, frames) {
  ...
  for (const conversion of options.conversions) {
    ...
    const matches = matchers.fieldMatchers.get(ids$1.FieldMatcherID.byName).get(conversion.targetField);
...
function convertFieldType(field, opts) {
  switch (opts.destinationType) {
    ...
    case dataFrame.FieldType.enum:
      return fieldToEnumField(field, opts.enumConfig);
    case dataFrame.FieldType.other:
...
function fieldToTimeField(field, dateFormat) {
  ...
  for (let t = 0; t < timeValues.length; t++) {
    if (timeValues[t]) {
      let parsed = isISO8601 ? Date.parse(timeValues[t]) : parser.dateTimeParse(timeValues[t], opts).valueOf();
...
function fieldToBooleanField(field) {
  ...
  return {
    ...field,
    type: dataFrame.FieldType.boolean,
...
function fieldToStringField(field, dateFormat, parseOptions, joinWith) {
  ...
  switch (field.type) {
    case dataFrame.FieldType.time:
      values = values.map((v) => parser.dateTimeParse(v, parseOptions).format(dateFormat));
      break;
...
function ensureTimeField(field, dateFormat) {
  ...
  if (field.type === dataFrame.FieldType.time && firstValueTypeIsNumber) {
    return field;
  }
...
function fieldToEnumField(field, config) {
  ...
  return {
    ...
    type: dataFrame.FieldType.enum,
    values: enumValues
  };
}

exports.convertFieldType = convertFieldType;
exports.convertFieldTypeTransformer = convertFieldTypeTransformer;
exports.convertFieldTypes = convertFieldTypes;
exports.ensureTimeField = ensureTimeField;
exports.fieldToStringField = fieldToStringField;
exports.fieldToTimeField = fieldToTimeField;
//# sourceMappingURL=convertFieldType.cjs.map
