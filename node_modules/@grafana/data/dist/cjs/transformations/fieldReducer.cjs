...
function getFieldTypeForReducer(id, fallback) {
  return id === "count" /* count */ || id === "distinctCount" /* distinctCount */ || id === "changeCount" /* changeCount */ || id === "countAll" /* countAll */ ? dataFrame.FieldType.number : id === "allIsNull" /* allIsNull */ || id === "allIsZero" /* allIsZero */ ? dataFrame.FieldType.boolean : fallback;
}
...
function reduceField(options) {
  ...
  const { nullValueMode = data.NullValueMode.Ignore } = field.config;
  const ignoreNulls = nullValueMode === data.NullValueMode.Ignore;
  const nullAsZero = nullValueMode === data.NullValueMode.AsZero;
...
const fieldReducers = new Registry.Registry(() => [
  {
  ...
  {
    id: "countAll" /* countAll */,
    name: "Count all",
    description: "Number of values (including empty)",
    emptyInputResult: 0,
    standard: false,
    reduce: (field) => ({ countAll: field.values.length }),
    preservesUnits: false
  },
  {
    id: "range" /* range */,
...
function doStandardCalcs(field, ignoreNulls, nullAsZero) {
  ...
  const isNumberField = field.type === dataFrame.FieldType.number || field.type === dataFrame.FieldType.time;
...
function calculateStdDev(field, ignoreNulls, nullAsZero) {
  if (!(field.type === dataFrame.FieldType.number || field.type === dataFrame.FieldType.time)) {
...
exports.getFieldTypeForReducer = getFieldTypeForReducer;
exports.isReducerID = isReducerID;
exports.reduceField = reduceField;
//# sourceMappingURL=fieldReducer.cjs.map
