...
const fieldNameMatcher = {
  ...
  get: (name) => {
    ...
    return (field, frame, allFrames) => {
      return name === field.name || name === fieldState.getFieldDisplayName(field, frame, allFrames) || Boolean(fallback && fallback(field, frame, allFrames));
    };
  },
...
const multipleFieldNamesMatcher = {
  ...
  get: (options) => {
    ...
    const matcher = (field, frame, frames) => {
      return uniqueNames.has(field.name) || uniqueNames.has(fieldState.getFieldDisplayName(field, frame, frames)) || Boolean(fallback && fallback(field, frame, frames));
    };
...
function fieldNameFallback(fields) {
  let fallback = void 0;
  if (fields.has(dataFrame.TIME_SERIES_VALUE_FIELD_NAME)) {
    fallback = (field, frame) => {
      var _a;
      return Boolean(field.labels) && // Value was reasonable when the name was set in labels or on the frame
      ((_a = field.labels) == null ? void 0 : _a.__name__) === field.name;
    };
  } else if (fields.has("Time") || fields.has("time")) {
    fallback = (field, frame) => {
      var _a;
      return ((_a = frame.meta) == null ? void 0 : _a.typeVersion) == null && field.type === dataFrame.FieldType.time;
    };
  }
  return fallback;
}
const regexpFieldNameMatcher = {
  ...
  get: (pattern) => {
    ...
    return (field, frame, allFrames) => {
      const displayName = fieldState.getFieldDisplayName(field, frame, allFrames);
...
const fieldsInFrameMatcher = {
  id: ids.FieldMatcherID.byFrameRefID,
...
const regexpOrMultipleNamesMatcher = {
  id: ids.FieldMatcherID.byRegexpOrNames,
...
const patternToRegex = (pattern) => {
  ...
  try {
    return string.stringToJsRegex(pattern);
  } catch (error) {
    console.error(error);
    return void 0;
  }
...
const frameNameMatcher = {
  id: ids.FrameMatcherID.byName,
  name: "Frame Name",
  description: "match the frame name",
  defaultOptions: "/.*/",
  get: (pattern) => {
    const regex = string.stringToJsRegex(pattern);
    return (frame) => {
...
exports.fieldNameFallback = fieldNameFallback;
exports.getFieldNameMatchers = getFieldNameMatchers;
exports.getFrameNameMatchers = getFrameNameMatchers;
//# sourceMappingURL=nameMatcher.cjs.map
