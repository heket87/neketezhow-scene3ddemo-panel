...
var standardTransformersRegistry = require('./standardTransformersRegistry.cjs');

"use strict";
const getOperator = (config, ctx) => (source) => {
  var _a, _b;
  const info = standardTransformersRegistry.standardTransformersRegistry.get(config.id);
  if (!info) {
    return source;
  }
  ...
  const matcher = ((_b = config.filter) == null ? void 0 : _b.options) ? matchers.getFrameMatchers(config.filter) : void 0;
  return source.pipe(
    operators.mergeMap(
...
//# sourceMappingURL=transformDataFrame.cjs.map
