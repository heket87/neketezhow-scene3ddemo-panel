...
var FieldConfigOptionsRegistry = require('../field/FieldConfigOptionsRegistry.cjs');
var plugin = require('../types/plugin.cjs');
var suggestions = require('../types/suggestions.cjs');
var OptionsUIBuilders = require('../utils/OptionsUIBuilders.cjs');
var deprecationWarning = require('../utils/deprecationWarning.cjs');
var registryFactories = require('./registryFactories.cjs');

"use strict";
class PanelPlugin extends plugin.GrafanaPlugin {
  constructor(panel) {
    super();
    ...
    this._initConfigRegistry = () => {
      return new FieldConfigOptionsRegistry.FieldConfigOptionsRegistry();
    };
    this.dataSupport = {
      annotations: false,
      alertStates: false
  ...
  get defaults() {
    ...
    if (!this._defaults && this.optionsSupplier) {
      const builder = new OptionsUIBuilders.PanelOptionsEditorBuilder();
  ...
  setDefaults(defaults) {
    deprecationWarning.deprecationWarning("PanelPlugin", "setDefaults", "setPanelOptions");
   ...
   */
  setEditor(editor) {
    deprecationWarning.deprecationWarning("PanelPlugin", "setEditor", "setPanelOptions");
    this.editor = editor;
    return this;
  }
  setNoPadding() {
   ...
   */
  setMigrationHandler(handler, shouldMigrate) {
  ...
  useFieldConfig(config = {}) {
    this._initConfigRegistry = () => registryFactories.createFieldConfigRegistry(config, this.meta.name);
    return this;
  }
   ...
   */
  getSuggestions(panelDataSummary) {
    var _a, _b;
    const withDefaults = (suggestion) => lodash.defaultsDeep(suggestion, {
    ...
    return (_b = (_a = this.suggestionsSupplier) == null ? void 0 : _a.call(this, panelDataSummary)) == null ? void 0 : _b.map(
      (s) => {
        const suggestionWithDefaults = withDefaults(s);
        return Object.assign(suggestionWithDefaults, { hash: suggestions.getSuggestionHash(suggestionWithDefaults) });
      }
    );
  }
  /**
  ...
  getSuggestionsSupplier() {
    ...
    return {
      getSuggestionsForData: (builder) => {
        var _a, _b;
        deprecationWarning.deprecationWarning("PanelPlugin", "getSuggestionsSupplier()", "getSuggestions(panelDataSummary)");
        (_b = (_a = this.suggestionsSupplier) == null ? void 0 : _a.call(this, builder.dataSummary)) == null ? void 0 : _b.forEach((s) => {
...
//# sourceMappingURL=PanelPlugin.cjs.map
