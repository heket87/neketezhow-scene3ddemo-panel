...
"use strict";
const createShadeSchema = (color) => z.z.enum([`super-light-${color}`, `light-${color}`, color, `semi-dark-${color}`, `dark-${color}`]);
const createHueSchema = (color) => z.z.object({
  name: z.z.literal(color),
  shades: z.z.array(
    z.z.object({
      color: z.z.string(),
      name: createShadeSchema(color),
      aliases: z.z.array(z.z.string()).optional(),
      primary: z.z.boolean().optional()
    })
  )
});
const ThemeVizHueSchema = z.z.union([
  createHueSchema("red"),
...
const ThemeVisualizationColorsInputSchema = z.z.object({
  hues: z.z.array(ThemeVizHueSchema).optional(),
  palette: z.z.array(z.z.string()).optional()
});
function createVisualizationColors(colors, options = {}) {
  ...
  const getColorByName = (colorName) => {
    if (!colorName) {
      return fieldColor.FALLBACK_COLOR;
    }
...
exports.ThemeVisualizationColorsInputSchema = ThemeVisualizationColorsInputSchema;
exports.createVisualizationColors = createVisualizationColors;
//# sourceMappingURL=createVisualizationColors.cjs.map
