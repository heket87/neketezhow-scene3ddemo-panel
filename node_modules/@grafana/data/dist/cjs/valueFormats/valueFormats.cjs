...
var categories$1 = require('./categories.cjs');
var dateTimeFormatters = require('./dateTimeFormatters.cjs');
var symbolFormatters = require('./symbolFormatters.cjs');
...
function buildFormats() {
  categories = categories$1.getCategories();
...
function getValueFormat(id) {
  ...
  if (!fmt && id) {
    ...
    if (idx > 0) {
      ...
      if (key === "time") {
        return dateTimeFormatters.toDateTimeValueFormatter(sub);
      }
      if (key === "si") {
        const offset = symbolFormatters.getOffsetFromSIPrefix(sub.charAt(0));
        const unit = offset === 0 ? sub : sub.substring(1);
        return symbolFormatters.SIPrefix(unit, offset);
      }
      if (key === "count") {
      ...
      if (key === "currency") {
        ...
        if (keySplit[0] === "financial" && keySplit.length >= 2) {
          ...
          return symbolFormatters.fullCurrency(symbol, asSuffix);
        } else {
          return symbolFormatters.currency(sub);
        }
      }
      if (key === "bool") {
        idx = sub.indexOf("/");
...
//# sourceMappingURL=valueFormats.cjs.map
