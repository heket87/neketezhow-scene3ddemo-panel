...
function getFrameDisplayName(frame, index) {
  ...
  if (index === void 0 && frame.fields.length > 0) {
    return frame.fields.filter((f) => f.type !== dataFrame.FieldType.time).map((f) => getFieldDisplayName(f, frame)).join(", ");
  }
  if (frame.refId) {
...
function decoupleHideFromState(frames, fieldConfig) {
  frames.forEach((frame) => {
    frame.fields.forEach((field) => {
      var _a, _b, _c;
      const hideFrom = {
        ...
        tooltip: false,
        viz: false,
        ...(_a = fieldConfig.defaults.custom) == null ? void 0 : _a.hideFrom
      };
      const hideFromState = (_b = field.config.custom) == null ? void 0 : _b.hideFrom;
      fieldConfig.overrides.forEach((o) => {
        if ("__systemRef" in o) {
          return;
        }
        const m = matchers.getFieldMatcher(o.matcher);
      ...
      field.config.custom = (_c = field.config.custom) != null ? _c : {};
...
function calculateFieldDisplayName(field, frame, allFrames) {
  ...
  if (field.type === dataFrame.FieldType.time && !field.labels) {
    return displayName != null ? displayName : dataFrame.TIME_SERIES_TIME_FIELD_NAME;
  }
  let parts = [];
  ...
  if (field.name && field.name !== dataFrame.TIME_SERIES_VALUE_FIELD_NAME) {
    parts.push(field.name);
  }
  if (field.labels && frame) {
    let singleLabelName = getSingleLabelName(allFrames != null ? allFrames : [frame]);
    if (!singleLabelName) {
      let allLabels = labels.formatLabels(field.labels);
      if (allLabels) {
  ...
  if (frame && !frameNameAdded && !labelsAdded && field.name === dataFrame.TIME_SERIES_VALUE_FIELD_NAME) {
  ...
  if (parts.length) {
    ...
    displayName = dataFrame.TIME_SERIES_VALUE_FIELD_NAME;
  }
...
exports.calculateFieldDisplayName = calculateFieldDisplayName;
exports.decoupleHideFromState = decoupleHideFromState;
...
//# sourceMappingURL=fieldState.cjs.map
