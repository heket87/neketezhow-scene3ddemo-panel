...
var fieldParser = require('../utils/fieldParser.cjs');
var FunctionalVector = require('../vector/FunctionalVector.cjs');
var processDataFrame = require('./processDataFrame.cjs');

"use strict";
const MISSING_VALUE = void 0;
class MutableDataFrame extends FunctionalVector.FunctionalVector {
  ...
  addFieldFor(value, name) {
    return this.addField({
      name: name || "",
      // Will be filled in
      type: processDataFrame.guessFieldTypeFromValue(value)
    });
  }
  addField(f, startLength) {
    ...
    if (!type && ("time" === f.name || "Time" === f.name)) {
      ...
      if (!type && buffer && buffer.length) {
        type = processDataFrame.guessFieldTypeFromValue(buffer[0]);
      }
      if (!type) {
    ...
    if (type === dataFrame.FieldType.other) {
      type = processDataFrame.guessFieldTypeForField(field);
      if (type) {
        field.type = type;
      }
    }
  ...
  parseValue(field, v) {
    ...
    if (!p) {
      p = this.setParser(field, fieldParser.makeFieldParser(v, field));
    }
    return p(v);
  }
  /**
  ...
  appendRow(row) {
    for (let i = this.fields.length; i < row.length; i++) {
      this.addField({
        name: `Field ${i + 1}`,
        type: processDataFrame.guessFieldTypeFromValue(row[i])
      });
    }
    if (this.length < 1) {
      for (let i = 0; i < this.fields.length; i++) {
        const f = this.fields[i];
        if (!f.type || f.type === dataFrame.FieldType.other) {
          f.type = processDataFrame.guessFieldTypeFromValue(row[i]);
        }
      }
    }
    for (let i = 0; i < this.fields.length; i++) {
      const f = this.fields[i];
      let v = row[i];
      if (f.type !== dataFrame.FieldType.string && lodash.isString(v)) {
  ...
  add(value) {
    ...
    for (const field of this.fields) {
      ...
      if (field.type !== dataFrame.FieldType.string && lodash.isString(val)) {
   ...
   */
  toJSON() {
    return processDataFrame.toDataFrameDTO(this);
  }
}

exports.MISSING_VALUE = MISSING_VALUE;
exports.MutableDataFrame = MutableDataFrame;
//# sourceMappingURL=MutableDataFrame.cjs.map
