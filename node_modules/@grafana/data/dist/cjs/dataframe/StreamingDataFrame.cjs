...
var joinDataFrames = require('../transformations/transformers/joinDataFrames.cjs');
var dataFrame = require('../types/dataFrame.cjs');
var labels = require('../utils/labels.cjs');
var legend = require('../utils/legend.cjs');
var DataFrameJSON = require('./DataFrameJSON.cjs');
...
var PushMode = /* @__PURE__ */ ((PushMode2) => {
  PushMode2[PushMode2["wide"] = 0] = "wide";
  PushMode2[PushMode2["labels"] = 1] = "labels";
  return PushMode2;
})(PushMode || {});
const _StreamingDataFrame = class _StreamingDataFrame {
  constructor(options) {
    ...
    this.serialize = (fieldPredicate, optionsOverride, trimValues) => {
      ...
      const dataFrameDTO = processDataFrame.toFilteredDataFrameDTO(this, fieldPredicate);
    ...
    this.initFromSerialized = (serialized) => {
      ...
      this.fields = serialized.fields.map((f) => {
        ...
        return {
          ...f,
          type: (_a = f.type) != null ? _a : dataFrame.FieldType.other,
  ...
  push(msg) {
    ...
    if (schema) {
      this.pushMode = 0 /* wide */;
      this.timeFieldIndex = schema.fields.findIndex((f) => f.type === dataFrame.FieldType.time);
      const firstField = schema.fields[0];
      if (this.timeFieldIndex === 1 && firstField.type === dataFrame.FieldType.string && (firstField.name === "labels" || firstField.name === "Labels")) {
      ...
      if (hasSameStructure(this.schemaFields, niceSchemaFields)) {
        ...
        if (displayNameFormat) {
          this.fields.forEach((f) => {
            ...
            f.config.displayNameFromDS = legend.renderLegendFormat(displayNameFormat, labels);
          });
        }
      } else {
        this.packetInfo.schemaChanged = true;
        ...
        this.fields = niceSchemaFields.map((f) => {
          ...
          if (displayNameFormat) {
            const labels = { [PROM_STYLE_METRIC_LABEL]: f.name, ...f.labels };
            config.displayNameFromDS = legend.renderLegendFormat(displayNameFormat, labels);
          }
          return {
            config,
            name: f.name,
            labels: f.labels,
            type: (_b = f.type) != null ? _b : dataFrame.FieldType.other,
    ...
    if (data && data.values.length && data.values[0].length) {
      ...
      if (entities) {
        entities.forEach((ents, i) => {
          if (ents) {
            DataFrameJSON.decodeFieldValueEntities(ents, values[i]);
          }
        });
      }
      if (this.pushMode === 1 /* labels */) {
        ...
        values = joinDataFrames.join(tables);
      }
      if (values.length !== this.fields.length) {
        ...
        this.fields = values.map((vals, idx) => {
          let name = `Field ${idx}`;
          let type = processDataFrame.guessFieldTypeFromValue(vals[0]);
          const isTime = idx === 0 && type === dataFrame.FieldType.number && vals[0] > 1600016688632;
          if (isTime) {
            type = dataFrame.FieldType.time;
            name = "Time";
          }
          return {
            name,
...
exports.getLastStreamingDataFramePacket = getLastStreamingDataFramePacket;
exports.getStreamingFrameOptions = getStreamingFrameOptions;
exports.parseLabelsFromField = parseLabelsFromField;
exports.transpose = transpose;
//# sourceMappingURL=StreamingDataFrame.cjs.map
