...
function arrayToDataFrame(source, names) {
  ...
  if (isObjectArray(source)) {
    ...
    df.fields.push(makeFieldFromValues(dataFrame.TIME_SERIES_VALUE_FIELD_NAME, source));
  }
  return df;
}
function makeFieldFromValues(name, values) {
  var _a;
  const f = { name, config: {}, values, type: dataFrame.FieldType.other };
  f.type = (_a = processDataFrame.guessFieldTypeForField(f)) != null ? _a : dataFrame.FieldType.other;
  return f;
}
...
//# sourceMappingURL=ArrayDataFrame.cjs.map
