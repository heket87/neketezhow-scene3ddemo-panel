...
var types = require('../events/types.cjs');
...
const hasLogsLabelTypesSupport = (datasource) => {
  if (!datasource || typeof datasource !== "object") {
    return false;
  }
...
const hasSupplementaryQuerySupport = (datasource, type, dsRequest) => {
  if (!datasource) {
    return false;
  }
  return ("getDataProvider" in datasource || "getSupplementaryRequest" in datasource) && "getSupplementaryQuery" in datasource && "getSupportedSupplementaryQueryTypes" in datasource && datasource.getSupportedSupplementaryQueryTypes(dsRequest).includes(type);
};
...
class LogSortOrderChangeEvent extends types.BusEventWithPayload {
}
...
exports.hasLogsLabelTypesSupport = hasLogsLabelTypesSupport;
