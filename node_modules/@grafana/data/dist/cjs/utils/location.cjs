...
var sanitize = require('../text/sanitize.cjs');
...
const getUrlForPartial = (location, searchParamsToUpdate) => {
  const searchParams = url.urlUtil.parseKeyValue(
  ...
  return assureBaseUrl(url.urlUtil.renderUrl(location.pathname, searchParams));
};
...
const locationUtil = {
  ...
  getTimeRangeUrlParams: () => {
    ...
    return url.urlUtil.toUrlParams(getTimeRangeUrlParams());
  },
  getVariablesUrlParams: (scopedVars) => {
    ...
    return url.urlUtil.toUrlParams(params);
  },
  getUrlForPartial,
  processUrl: (url) => {
    return grafanaConfig.disableSanitizeHtml ? url : sanitize.textUtil.sanitizeUrl(url);
  }
};
