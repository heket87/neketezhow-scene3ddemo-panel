...
const timeZoneFormatUserFriendly = (timeZone) => {
  switch (common.getTimeZone({ timeZone })) {
    case "browser":
...
const getZone = (timeZone) => {
  return moment__default.default.tz.zone(timeZone);
};
...
const getTimeZones = lodash.memoize((includeInternal = false) => {
  ...
  return moment__default.default.tz.names().reduce((zones, zone) => {
...
const mapInternal = (zone, timestamp) => {
  ...
  switch (zone) {
    ...
    case "browser" /* localBrowserTime */: {
      const tz = moment__default.default.tz.guess(true);
...
const mapToInfo = (timeZone, timestamp) => {
  var _a;
  const momentTz = moment__default.default.tz.zone(timeZone);
  if (!momentTz) {
    return void 0;
  }
  return {
...
const countriesByTimeZone = (() => {
  return moment__default.default.tz.countries().reduce((all, code) => {
    const timeZones = moment__default.default.tz.zonesForCountry(code);
