...
var UPlotConfigBuilder = require('../../components/uPlot/config/UPlotConfigBuilder.cjs');
var gradientFills = require('../../components/uPlot/config/gradientFills.cjs');
var utils = require('../../components/uPlot/utils.cjs');
var utils$1 = require('../GraphNG/utils.cjs');
...
const preparePlotConfigBuilder = ({
  ...
  const builder = new UPlotConfigBuilder.UPlotConfigBuilder(timeZones[0]);
  let alignedFrame;
  builder.setPrepData((frames) => {
    alignedFrame = frames[0];
    return utils.preparePlotData2(frames[0], builder.getStackingGroups());
  });
  ...
  for (let i = 1; i < frame.fields.length; i++) {
    ...
    const scaleKey = utils$1.buildScaleKey(config, field.type);
    ...
    if (customConfig.axisPlacement !== schema.AxisPlacement.Hidden) {
      ...
      if (customConfig.axisColorMode === schema.AxisColorMode.Series) {
        if (colorMode.isByValue && ((_o = field.config.custom) == null ? void 0 : _o.gradientMode) === schema.GraphGradientMode.Scheme && colorMode.id === data.FieldColorModeId.Thresholds) {
          axisColor = gradientFills.getScaleGradientFn(1, theme, colorMode, field.config.thresholds);
        } else {
  ...
  let stackingGroups = utils.getStackingGroups(frame);
...
exports.getNamesToFieldIndex = getNamesToFieldIndex;
exports.preparePlotConfigBuilder = preparePlotConfigBuilder;
