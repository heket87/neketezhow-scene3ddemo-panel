...
var Typeahead = require('../components/Typeahead/Typeahead.cjs');
var searchFunctions = require('../utils/searchFunctions.cjs');
var slate = require('../utils/slate.cjs');
var TOKEN_MARK = require('./slate-prism/TOKEN_MARK.cjs');
    ...
    commands: {
      ...
      applyTypeahead: (editor, suggestion) => {
        ...
        if (suggestionText.match(/\n/)) {
          const fragment = slate.makeFragment(suggestionText);
...
const handleTypeahead = async (editor, onStateChange, onTypeahead, cleanText) => {
  ...
  const filteredSuggestions = suggestions.map((group) => {
    ...
    const searchFunctionType = group.searchFunctionType || (group.prefixMatch ? searchFunctions.SearchFunctionType.Prefix : searchFunctions.SearchFunctionType.Word);
    const searchFunction = searchFunctions.SearchFunctionMap[searchFunctionType];
    let newGroup = { ...group };
    if (prefix) {
      if (!group.skipFilter) {
...
exports.TYPEAHEAD_DEBOUNCE = TYPEAHEAD_DEBOUNCE;
exports.getNumCharsToDelete = getNumCharsToDelete;
