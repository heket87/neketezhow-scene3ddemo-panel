...
const getColorIndicatorClass = (colorIndicator, styles) => {
  switch (colorIndicator) {
    case types.ColorIndicator.series:
      return styles.series;
    case types.ColorIndicator.value:
      return styles.value;
    case types.ColorIndicator.hexagon:
      return styles.hexagon;
    case types.ColorIndicator.pie_1_4:
      return styles.pie_1_4;
    case types.ColorIndicator.pie_2_4:
      return styles.pie_2_4;
    case types.ColorIndicator.pie_3_4:
      return styles.pie_3_4;
    case types.ColorIndicator.marker_sm:
      return styles.marker_sm;
    case types.ColorIndicator.marker_md:
      return styles.marker_md;
    case types.ColorIndicator.marker_lg:
...
const getTooltipDisplayValue = (value, field) => {
  ...
  return { text: data.formattedValueToString(display), numeric: display.numeric, color: display.color };
};
const getContentItems = (fields, xField, dataIdxs, seriesIdx, mode, sortOrder, fieldFilter = (field) => true, hideZeros = false, _restFields) => {
  var _a, _b, _c, _d, _e, _f;
  let rows = [];
  let allNumeric = true;
  for (let i = 0; i < fields.length; i++) {
    const field = fields[i];
    if (field === xField || field.type === data.FieldType.time || !fieldFilter(field) || ((_b = (_a = field.config.custom) == null ? void 0 : _a.hideFrom) == null ? void 0 : _b.tooltip)) {
      continue;
    }
    ...
    rows.push({
      label: (_d = (_c = field.state) == null ? void 0 : _c.displayName) != null ? _d : field.name,
      value: display.text,
      color: (_e = display.color) != null ? _e : data.FALLBACK_COLOR,
      colorIndicator,
      colorPlacement,
...
const getIndicatorAndPlacement = (field) => {
  ...
  let colorIndicator = types.ColorIndicator.series;
  let colorPlacement = types.ColorPlacement.first;
  if (colorMode.isByValue) {
    colorIndicator = types.ColorIndicator.value;
    colorPlacement = types.ColorPlacement.trailing;
  }
  return { colorIndicator, colorPlacement };
};

exports.calculateTooltipPosition = calculateTooltipPosition;
exports.getColorIndicatorClass = getColorIndicatorClass;
