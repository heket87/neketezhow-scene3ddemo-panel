...
import { formattedValueToString, GAUGE_DEFAULT_MINIMUM, GAUGE_DEFAULT_MAXIMUM, ThresholdsMode, FieldColorModeId, FALLBACK_COLOR } from '@grafana/data';
...
import { calculateGaugeAutoProps, getFormattedThresholds, DEFAULT_THRESHOLDS } from './utils.mjs';
...
class Gauge extends PureComponent {
  ...
  draw() {
    ...
    let thresholds = [];
    if (((_f = field.color) == null ? void 0 : _f.mode) === FieldColorModeId.Thresholds) {
      thresholds = getFormattedThresholds(decimals, field, theme);
    } else {
      thresholds = [{ value: (_g = field.min) != null ? _g : GAUGE_DEFAULT_MINIMUM, color: (_h = value.color) != null ? _h : FALLBACK_COLOR }];
    }
    const options = {
      series: {
        gauges: {
          gauge: {
            min,
            max,
            neutralValue: (_i = field.custom) == null ? void 0 : _i.neutral,
