import { FieldType, isLikelyAscendingVector, sortDataFrame, applyNullInsertThreshold, guessDecimals, roundDecimals, nullToValue, getFieldColorModeForField } from '@grafana/data';
import { t } from '@grafana/i18n';
import { AxisPlacement, VisibilityMode, GraphDrawStyle, ScaleDirection, ScaleOrientation } from '@grafana/schema';
...
function getYRange(alignedFrame) {
  ...
  const decimals = (_e = field.config.decimals) != null ? _e : Math.max(guessDecimals(min), guessDecimals(max));
...
const prepareConfig = (sparkline, dataFrame, theme, showHighlights) => {
  ...
  for (let i = 0; i < dataFrame.fields.length; i++) {
    ...
    const pointsMode = customConfig.drawStyle === GraphDrawStyle.Points || hasHighlightIndex ? VisibilityMode.Always : customConfig.showPoints;
    builder.addSeries({
      pxAlign: false,
      scaleKey,
      theme,
...
export { getYRange, prepareConfig, preparePlotFrame, prepareSeries };
